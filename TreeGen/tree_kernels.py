...
class Kernel():
    ...
    def printKernelEvaluation(self,dataset,file):
        if not isinstance(dataset, tree.Dataset):
            print "ERROR: the first Parameter must be a Dataset object"
            return
        kernel = 0
        ...
        for i in range(ne-1):
            for j in range(i+1,ne):
               # print"( %d , %d )"%(i,j)
                ...
                kernel += self.kernel(dataset.getExample(i), dataset.getExample(j))
                t_end = datetime.datetime.now()
                t += (t_end - t_start).microseconds
        file.write(str((kernel/size))+" : "+str(t/size))
